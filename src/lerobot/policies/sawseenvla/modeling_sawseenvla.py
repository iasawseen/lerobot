...
Example of training SawSeenVLA from a fresh action expert (loads only the
...
Example of using a trained SawSeenVLA checkpoint:
```python
policy = SawSeenVLAPolicy.from_pretrained("<USER>/<sawseenvla_checkpoint>")
```
"""
...
from .configuration_sawseenvla import SawSeenVLAConfig
...
def aloha_gripper_to_angular(value):
    ...
    # reverses this transformation to be consistent with sawseenvla which is pretrained in
    # angular space.
    #
...
def aloha_gripper_from_angular(value):
    # Convert from the gripper position used by sawseenvla to the gripper position that is used by Aloha.
...
class SawSeenVLAPolicy(PreTrainedPolicy):
    """Wrapper class around VLAFlowMatching model to train and run inference within LeRobot."""

    config_class = SawSeenVLAConfig
    name = "sawseenvla"

    def __init__(
        self,
        config: SawSeenVLAConfig,
        **kwargs,
    ):
        """
        Args:
    ...
    def prepare_images(self, batch):
        """Apply SawSeenVLA preprocessing to the images, like resizing to 224x224 and padding to keep aspect ratio, and
...
class VLAFlowMatching(nn.Module):
    """
    SawSeenVLA flow-matching action expert.
    ...
    def __init__(self, config: SawSeenVLAConfig, rtc_processor: RTCProcessor | None = None):
        super().__init__()
        self.config = config
